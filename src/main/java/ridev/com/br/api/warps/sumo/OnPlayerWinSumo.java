...
public class OnPlayerWinSumo extends Event implements Cancellable {
    ...
    public OnPlayerWinSumo(User winner, User loser) {
        this.winner = winner;
        this.loser = loser;
        CombatLogAPI.removePlayerCombat(winner.getPlayer());
        CombatLogAPI.removePlayerCombat(loser.getPlayer());
        Sumo.inDuel.remove(winner);
        Sumo.inDuel.remove(loser);
        winner.getPlayer().sendMessage(FancyText.colored("&a&lSUMO &8➸ &7Você ganhou a batalha contra &c" + loser.getUsername() + "&7!"));
        loser.getPlayer().sendMessage(FancyText.colored("&a&lSUMO &8➸ &7Você perdeu a batalha contra &c" + winner.getUsername() + "&7!"));

        UserRecompenses.giveRecompenses(winner, loser);
        respawnUsers(WarpLibrary.getWarp(WarpType.SUMO).getSpawn());
    }
    ...
    public void respawnUsers(Location loc) {
        ...
        new BukkitRunnable() {
            public void run() {
                ...
                SumoItens.setIntens(winner.getPlayer());
                SumoItens.setIntens(loser.getPlayer());
